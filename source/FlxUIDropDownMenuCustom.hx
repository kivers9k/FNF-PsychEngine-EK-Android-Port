		...
		if (dropPanel.visible)
		{
			...
			if(list.length > 1 && canScroll) 
			{
				for (swipe in FlxG.swipes)
				{
					...
					{
						if ((FlxG.mouse.wheel < 0 || FlxG.keys.justPressed.DOWN || -45 <= swipe.startPosition.angleBetween(swipe.endPosition) && 45 >= swipe.startPosition.angleBetween(swipe.endPosition)))
						{
							// Go down
							currentScroll++;
						...
						else if (FlxG.mouse.wheel > 0 || FlxG.keys.justPressed.UP || -180 <= swipe.startPosition.angleBetween(swipe.endPosition) && -135 >= swipe.startPosition.angleBetween(swipe.endPosition) || (135 <= swipe.startPosition.angleBetween(swipe.endPosition) && 180 >= swipe.startPosition.angleBetween(swipe.endPosition)))
						{
							// Go up
							--currentScroll;
